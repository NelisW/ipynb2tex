...
\item \verb+-l+ [optional] (the lower case letter el) if this option is given the code listings are floated to the end of the document. Otherwise the code listings are placed in the document where they occur in the notebook.
\end{itemize}
...
where the symbolic link is constructed using the cell number and the text used in the \verb+{purpose}+ part is taken from the first line of the code, if the first line is a comment.  Hence if the code in cell 3 starts with the lines
...
See Listing~\ref{lst:listing1} for the code.
...
See Listing~\ref{lst:listing2} for the code.
...
See Listing~\ref{lst:listing3} for the code.
...
See Listing~\ref{lst:autolistingcell36} for the code.
...
See Listing~\ref{lst:autolistingcell38} for the code.
...
See Listing~\ref{lst:figurelisting} for the code.


\begin{figure}[tb]
\centering
\includegraphics{./pic/test2LaTeX_42_0.png}
...
See Listing~\ref{lst:autolistingcell44} for the code this figure is not given meta data caption parameters, it will be inlined in the latex export.

\begin{center}
\includegraphics{./pic/test2LaTeX_44_0.png}
\end{center}
...
See Listing~\ref{lst:autolistingcell46} for the code.


\begin{figure}[tb]
\centering
\includegraphics{./pic/test2LaTeX_46_0.png}
...
See Listing~\ref{lst:autolistingcell48} for the code.


\begin{figure}[tb]
\centering
\includegraphics{./pic/test2LaTeX_48_0.png}
\caption{Caption text for (first) fourth figure\label{fig:lab4-0}}
\end{figure}

\begin{center}
\includegraphics{./pic/test2LaTeX_48_1.png}
\end{center}


\begin{figure}[tb]
\centering
\includegraphics{./pic/test2LaTeX_48_2.png}
...
See Listing~\ref{lst:autolistingcell56} for the code  type(eval("this is a test string")) is not list.
...
There are some really nice videos\cite{httpswwwyoutubecomwatchvaIXED26Wppg} on YouTube.
...
See Listing~\ref{lst:autolistingcell70} for the code.
...
The probability density function for yaw angle is the wrapped normal distribution\cite{httpsenwikipediaorgwikiWrappednormaldistribution}.  The first form uses two dollar symbols  to show the equation in LaTeX display math:
...
From: Introduction to scientific computing with Python\cite{httpnbvieweripythonorggithubjrjohanssonscientificpythonlecturesblobmasterLecture0ScientificComputingwithPythonipynb} we learn:
...
pandas&0.18.0\\\hline
\multicolumn{2}{|c|}{Tue Apr 26 12:12:56 2016 South Africa Standard Time}\\\hline
...
matplotlib 1.5.1
pandas 0.18.0
Tue Apr 26 12:12:56 2016 South Africa Standard Time
\end{lstlisting}
...
\begin{lstlisting}[style=incellstyle,caption={Caption text for second listing \label{lst:listing2}}]
#
print(' ')
...
\begin{lstlisting}[style=incellstyle,caption={Code Listing in cell 36 \label{lst:autolistingcell36}}]
print('code with no listing caption')
\end{lstlisting}


\begin{lstlisting}[style=incellstyle,caption={Code Listing in cell 38 \label{lst:autolistingcell38}}]
def doprint(i):
...
\begin{lstlisting}[style=incellstyle,caption={Code Listing in cell 70 \label{lst:autolistingcell70}}]
import pandas as pd
