...
The company logo can be supplied in the form of a png file.  This file must be defined in the command:\\
\verb+\WPdocLogo{logo.png}+\\
If the command is not used or is empty, no image is shown. If it is not empty, the contents must be a path to a png picture file to be used as logo.  The picture fits flush to the left margin, so if the logo is only required on the right side, use lots of white or transparency on the left side.
